...
class DonationFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val locationCard = view.findViewById<CardView>(R.id.locationCard)
        locationCard.setOnClickListener {
            // Programmatically "tap" the Location bottom nav item
            val bottomNav = requireActivity().findViewById<BottomNavigationView>(R.id.bottom_navigation)
            bottomNav.selectedItemId = R.id.nav_location
        }

        val supportButton = view.findViewById<MaterialButton>(R.id.supportButton)
        supportButton.setOnClickListener {
            parentFragmentManager.beginTransaction()
                .replace(R.id.fragment_container, MoneyDonationFragment())
                .addToBackStack(null)
                .commit()
        }
